...
**稼働中スクレイパー**: 18ソース（詳細は config/scrapers.yaml 参照）
**テスト**: 46テスト全て成功（pytest実行済み）
...
### 一時的に無効化（skip済み）スクレイパー

以下のスクレイパーは config/scrapers.yaml で skip: true に設定されており、実行時にスキップされます：

- **ポケモンセンター公式** - 公式サイトのBot対策強化により、正確な情報取得が困難に（将来対応予定）
- **ヨドバシカメラ** - サイト更新により DOM構造が変更（再実装予定）
- **エディオン** - サイト設定変更により取得困難に
- **ケーズデンキ** - 在庫管理システムの変更対応待機中
- **Playwright版スクレイパー** - 環境依存問題のため Headless ブラウザ版は無効化（BeautifulSoup版で対応）

これらは今後のメンテナンスで復活予定です。稼働中のスクレイパー（18ソース）で主要な情報をカバーしています。
...
- [x] コード品質改善（PEP8準拠 import、DeprecationWarning対応）
- [x] utils.py作成（共通ユーティリティ関数集約）【cmd_247】
- [x] HTMLレポートにテーブル＋ソート機能追加【cmd_247p2】
